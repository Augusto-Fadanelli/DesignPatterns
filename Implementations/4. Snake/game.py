...
class PlayGame:
    def __init__(
        self, snake: Snake, food: Food, portal: Portal, caretaker: Caretaker
    ):
    ...
    def __reloadGame(self):
        self.snake.setPos(
            self.screen_width // 2 - self.snake.getRectSize() // 2,
            self.screen_height // 2 - self.snake.getRectSize() // 2,
        )
        self.snake.setControl(self.snake.getSpeed(), 0)
        self.score = 0
        self.food.setPos(
            randint(
                self.food.getRadius() * 2,
                self.screen_width - self.food.getRadius() * 2,
            ),
            randint(
                self.food.getRadius() * 2,
                self.screen_height - self.food.getRadius() * 2,
            ),
        )
        ...
        while True:
            ...
            for event in pygame.event.get():
                ...
                if event.type == KEYDOWN:
                    if event.key == K_a:
                        if (
                            not self.snake.getControlX()
                            == self.snake.getSpeed()
                        ):   # Impede a snake de inverter a direção
                            self.snake.setControl(-self.snake.getSpeed(), 0)
                    if event.key == K_d:
                        if (
                            not self.snake.getControlX()
            ...
            # Snake Shape
            snake_head = pygame.draw.rect(
                self.screen,
                (0, 255, 0),
                (
                    self.snake.getPosX(),
                    self.snake.getPosY(),
                    self.snake.getRectSize(),
                    self.snake.getRectSize(),
                ),
            )

            # Portal Shape
            portal = pygame.draw.circle(
                self.screen,
                (0, 0, 255),
                (self.portal.getPosX(0), self.portal.getPosY(0)),
                self.portal.getRadius(),
            )
            portal = pygame.draw.circle(
                self.screen,
                (0, 0, 255),
                (self.portal.getPosX(1), self.portal.getPosY(1)),
                self.portal.getRadius(),
            )
            portal = pygame.draw.circle(
                self.screen,
                (0, 0, 255),
                (self.portal.getPosX(2), self.portal.getPosY(2)),
                self.portal.getRadius(),
            )

            # Food Shape
            food = pygame.draw.circle(
                self.screen,
                (255, 0, 0),
                (self.food.getPosX(), self.food.getPosY()),
                self.food.getRadius(),
            )
            ...
            if self.snake.checkCollision():
                ...
                formatted_game_over = self.font_game_over.render(
                    message_game_over, True, (0, 0, 0)
                )
                ...
                while self.snake.getIsDead():
                    ...
                    pos_message_game_over.center = (
                        self.screen_width // 2,
                        self.screen_height // 2,
                    )
                    self.screen.blit(
                        formatted_game_over, pos_message_game_over
                    )
                    pygame.display.update()

            # Se a snake sair da tela
            if self.snake.getPosX() < 0:
                self.snake.setPos(self.screen_width, self.snake.getPosY())
            elif self.snake.getPosX() > self.screen_width:
                self.snake.setPos(0, self.snake.getPosY())
            if self.snake.getPosY() < 0:
                self.snake.setPos(self.snake.getPosX(), self.screen_height)
            elif self.snake.getPosY() > self.screen_height:
                self.snake.setPos(self.snake.getPosX(), 0)
            ...
            formatted_score = self.font_score.render(
                message_score, False, (0, 0, 0)
            )
