...
print(
    f'Começando a criar os mago em: {datetime.datetime.now().time()}\n',
)
for i in range(3):
    mago = Mage(23, 90, 70)
    print(
        f'Terminou de criar o {i+1} mago em: {datetime.datetime.now().time()}'
    )
print(
    f'Criação de todos os {i+1} magos terminou em: {datetime.datetime.now().time()}\n'
)
print(
    'Atributos: ' + ', '.join('%s: %s' % item for item in vars(mago).items())
)
...
print(
    f'Começando a clonar varios elfos em: {datetime.datetime.now().time()}\n'
)
elfo = SummonerElf(948, 67, 65)
for i in range(20):
    elfo_clones = elfo.clone()
    print(
        f'Terminado de criar {i+1} clone de elfo em: {datetime.datetime.now().time()}'
    )
print(
    f'Terminou de criar a população elfos em: {datetime.datetime.now().time()}\n'
)
print(
    'Atributos: ' + ', '.join('%s: %s' % item for item in vars(elfo).items())
)
